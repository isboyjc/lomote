...
export async function useConfig() {
  ...
  return computed(
    () => {
      ...
      const getNavKeyFromPath = (path: string, key: string, items: any[]) => {
        const item = items.find(item => item._path === path);
        return item?.[key] || {};
      };
      ...
      return {
        ...
        header: {
          ...header,
          ...getNavKeyFromPath(route.path, 'header', navigation.value || []),
          ...page.value?.header,
        } as (typeof header & DefaultConfig['header']),
        banner: {
          ...banner,
          ...getNavKeyFromPath(route.path, 'banner', navigation.value || []),
          ...page.value?.banner,
        } as (typeof banner & DefaultConfig['banner']),
        main: {
          ...main,
          ...getNavKeyFromPath(route.path, 'main', navigation.value || []),
          ...page.value?.main,
        } as (typeof main & DefaultConfig['main']),
        aside: {
          ...aside,
          ...getNavKeyFromPath(route.path, 'aside', navigation.value || []),
          ...page.value?.aside,
        } as (typeof aside & DefaultConfig['aside']),
        toc: {
          ...toc,
          ...getNavKeyFromPath(route.path, 'toc', navigation.value || []),
          ...page.value?.toc,
        } as (typeof toc & DefaultConfig['toc']),
        footer: {
          ...footer,
          ...getNavKeyFromPath(route.path, 'footer', navigation.value || []),
          ...page.value?.footer,
